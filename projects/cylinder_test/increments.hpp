#ifndef INCREMENTS_HPP
#define INCREMENTS_HPP
...
std::vector<Eigen::VectorXd> increments(
    ...
    std::function<Eigen::VectorXd(double, Eigen::VectorXd, double, unsigned i)> f,
  ...
  for (unsigned i = 0; i < number_stages; ++i) {
    ...
    Eigen::VectorXd rhs = timestep *  f(increment_time, increment_solution, t_0, i) +
    ...
    if (debug){
        ...
        Eigen::VectorXd feval = timestep * f(increment_time, increment_solution, t_0, i);
        nodal_data = lf::mesh::utils::make_CodimMeshDataSet<double>(mesh_p, 2);
        for (int global_idx = 0; global_idx < increment.rows(); global_idx++) {
