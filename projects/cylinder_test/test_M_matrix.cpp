...
//   Eigen::MatrixXd quadrature_points (7, 3); 
//   quadrature_points <<  0.33333333, 0.33333333, 0.3333333, 
...
//   Eigen::VectorXd quadrature_weights(7);
//   quadrature_weights << 0.22500000, 0.12593918, 0.12593918, 0.12593918, 0.13239415, 0.13239415, 0.13239415;

//   Eigen::Matrix3d hard_coded_mat;
//   hard_coded_mat.setZero();
...
//   // Modify the lambda declarations to explicitly specify their return type
//   auto lambda1 = [&V, area](const Vec2d& x) -> double {
//       Vec2d temp;
//       temp << V(1,1) - V(1,2), V(0,2) - V(0,1);
//       return (x - V.col(1)).dot(temp) / (2 * area);
//   };

//   auto lambda2 = [&V, area](const Vec2d& x) -> double {
//       Vec2d temp;
//       temp << V(1,2) - V(1,0), V(0,0) - V(0,2);
//       return (x - V.col(2)).dot(temp) / (2 * area);
//   };

//   auto lambda3 = [&V, area](const Vec2d& x) -> double {
//       Vec2d temp;
//       temp << V(1,0) - V(1,1), V(0,1) - V(0,0);
//       return (x - V.col(0)).dot(temp) / (2 * area);
//   };

//   // Create the vector and store the lambdas
//   std::vector<std::function<double(const Vec2d&)>> lambdas;
...
//         eval_point << quadrature_points.row(k).dot(V.row(0)), quadrature_points.row(k).dot(V.row(1));
//         hard_coded_mat(i, j) += lambdas[i](eval_point) * lambdas[j](eval_point) * conductivity * quadrature_weights[k] * area; 
//         if (i != j){
//           hard_coded_mat(j, i) += lambdas[i](eval_point) * lambdas[j](eval_point) * conductivity * quadrature_weights[k] * area; 
//         }
//       }
//     }
//   }
