...
int main (int argc, char *argv[]){
    ...
    for (unsigned i = 0; time <= total_time; ++i, time += step_size){
        ...
        for (unsigned j = 1; newton_residual >  newton_tolerance; ++j){ //j < newton_steps
            ...
            std::cout << "currnte newton step - current timestep norm : " << (current_newton_step - current_timestep).norm() << std::endl;
            ...
            if (rhs.lpNorm<Eigen::Infinity>() > 1e-15) {
                std::cout << "newton residual " << newton_residual << std::endl; 
                ...
                std::cout << "Left hand side - rhs : " << (lhs * next_newton_step - rhs).lpNorm<Eigen::Infinity>() << std::endl;
            }
        ...
        // Eigen::VectorXd next_timestep = implicit_euler_step(A_temp,  M_temp, step_size, current_timestep,  phi_temp); //extended
        ...
        std::cout << "Right hand side : " << rhs.lpNorm<Eigen::Infinity>() << std::endl; 
        std::cout << "Left hand side : " << (lhs * next_timestep).lpNorm<Eigen::Infinity>() << std::endl; 
